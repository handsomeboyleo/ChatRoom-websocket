...
const port = 5050; //端口号
const cert = "chatroom"; //Token加密secret签名
 ...
 */
const fsReadAlllog = (callback) =>
    fs.readFile("./AllMsgs.json", "utf-8", callback);
/**
 * 将消息写入本地聊天记录Json
 * @params message
 */
const fsSaveMsgsLog = (msg) =>
    fsReadAlllog((err, data) => {
        if (err) {
 ...
 * @params callback(error,data)
 */
const fsReadUsers = (callback) =>
    fs.readFile("./Account.json", "utf-8", callback);
/**
 * 将用户写入本地用户Json
 * @params message
 */
const fsSaveUser = (user) =>
    fsReadUsers((err, data) => {
        if (err) {
            return console.error(err);
        }
...
const httpServer = http.createServer((req, res) => {
    ...
    res.setHeader(
        ...
        "X-Requested-With,X_Requested_With,Content-Type,,Authorization"
    );
            ...
            fsReadUsers(function (error, data) {
                ...
                if (exist) {
                    const body = JSON.stringify({
                        code: "200",
                        data: false,
                        message: (existUserName && "existUserName") ||
        ...
        if (req.url == "/login") {
            ...
            fsReadUsers(function (error, data) {
                ...
                const isLogin = users.some(
                    (user) =>
                    user.email === reqbody.account && user.password === reqbody.password
                );
                const current =
                    isLogin && users.find((user) => user.email === reqbody.account);
                const token = current && generateToken(current.userName);
                const body = JSON.stringify({
                    code: "200",
                    data: isLogin ? {
                        token,
...
const websocketServer = new webSocket.Server({
    ...
    () => {
        console.log(
            ` -- %cwebsocketServer : http://${hostname}:${port}/ `,
            "color:blue"
        );
    }
);
